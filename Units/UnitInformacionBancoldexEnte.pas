unit UnitInformacionBancoldexEnte;
...
type
  TfrmInformacionBancoldexEnte = class(TForm)
    Panel2: TPanel;
...
var
  frmInformacionBancoldexEnte: TfrmInformacionBancoldexEnte;
...
procedure TfrmInformacionBancoldexEnte.BitBtn1Click(Sender: TObject);
begin
...
procedure TfrmInformacionBancoldexEnte.FormShow(Sender: TObject);
begin
...
procedure TfrmInformacionBancoldexEnte.FormCreate(Sender: TObject);
begin
        IBLineaB.Open;
        IBLineaB.Last;
end;

procedure TfrmInformacionBancoldexEnte.CBLineaBExit(Sender: TObject);
begin
        EdSaldoB.Value := IBLineaB.FieldByName('SALDO').AsCurrency;
        NoBancoldex := IBLineaB.FieldByName('ID_OBLIGACION').AsString;
end;

procedure TfrmInformacionBancoldexEnte.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self);
end;
