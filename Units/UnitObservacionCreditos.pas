unit UnitObservacionCreditos;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, JvLabel, Mask, ExtCtrls, JvStaticText, Buttons, UnitDmGeneral;

type
  TFrmObservacionCreditos = class(TForm)
    Panel1: TPanel;
...
var
  FrmObservacionCreditos: TFrmObservacionCreditos;
...
procedure TFrmObservacionCreditos.MKsolicitudKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmObservacionCreditos.MKsolicitudExit(Sender: TObject);
begin
...
procedure TFrmObservacionCreditos.FormCreate(Sender: TObject);
begin
...
procedure TFrmObservacionCreditos.cerrarClick(Sender: TObject);
begin
        Close;
end;

procedure TFrmObservacionCreditos.nuevoClick(Sender: TObject);
begin
        MKsolicitud.Text := '';
        JVnombre.Caption := '';
        JVfecha.Caption := '';
...
procedure TFrmObservacionCreditos.Memo1Change(Sender: TObject);
begin
        //Memo1.Text := UpperCase(Memo1.Text);
end;

procedure TFrmObservacionCreditos.Memo1Exit(Sender: TObject);
begin
        Memo1.Text := UpperCase(Memo1.Text);
end;

procedure TFrmObservacionCreditos.BitBtn3Click(Sender: TObject);
begin
          if opcion then
          begin
          with DmSolicitud.IBSolicitud1 do
