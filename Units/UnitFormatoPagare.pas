unit UnitFormatoPagare;

interface

uses
  Windows, Messages, Printers, StrUtils, Math, DateUtils, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, FR_Class, StdCtrls, Buttons, ExtCtrls,
  JvEdit, JvTypedEdit, IBSQL, FR_Rich, NLetra, FR_Shape, DB,
  IBCustomDataSet, IBQuery, DBClient, Grids, FR_DSet, FR_DBSet;

type
  TfrmFormatoPagare = class(TForm)
    Reporte: TfrReport;
    Label1: TLabel;
    Panel1: TPanel;
    CmdCerrar: TBitBtn;
...
var
  frmFormatoPagare: TfrmFormatoPagare;
  ListaDeudores:TList;
...
procedure TfrmFormatoPagare.CmdCerrarClick(Sender: TObject);
begin
        Close;
end;

procedure TfrmFormatoPagare.CmdImprimirClick(Sender: TObject);
...
procedure TfrmFormatoPagare.EdColocacionKeyPress(Sender: TObject;
  var Key: Char);
begin
        NumericoSinPunto(Sender,Key);
end;

procedure TfrmFormatoPagare.ReporteGetValue(const ParName: String;
  var ParValue: Variant);
var i,Total:Integer;
    ADeudores:PDeudores;
...
begin
        ...
        if ParName = 'DEUDORES' then begin
         ...
         if ParName = 'FIRMAS' then begin
           ...
           begin
            if i = 0 then
              Texto := Texto + 'DEUDOR' + #13 + 'C.C. No.'+ ESPACIOS + ESPACIOS + ESPACIOS + ESPACIOS + ESPACIOS + 'HUELLA'
            else
              Texto := Texto + SALTO + 'CODEUDOR' + #13 + 'C.C. No.' + ESPACIOS + ESPACIOS + ESPACIOS + ESPACIOS + ESPACIOS + 'HUELLA';
           end;
           ParValue := Texto;
         end;
       ...
       begin  // begin 1
        ...
        if ParName = 'TASA_DE_INTERES' then begin
          ...
          if IBSQLPagare.FieldByName('ID_LINEA').AsInteger <> 6 then
...
procedure TfrmFormatoPagare.FormKeyPress(Sender: TObject; var Key: Char);
begin
        EnterTabs(Key,Self);
end;
