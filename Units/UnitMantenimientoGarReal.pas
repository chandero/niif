...
type
  TfrmMantenimientoGarReal = class(TForm)
    DSReal: TDataSource;
    Panel1: TPanel;
    Label1: TLabel;
    ...
    CDGarRealES_HIPOTECA: TBooleanField;
...
var
  frmMantenimientoGarReal: TfrmMantenimientoGarReal;
  vcolocacion : string;
implementation
...
procedure TfrmMantenimientoGarReal.EdNumeroColocacionExit(Sender: TObject);
begin
        ...
        with IBSQL1 do Begin
          ...
          if RecordCount = 0 then
           ...
           while not Eof do begin
            ...
            CDGarReal.FieldValues['ES_HIPOTECA'] := FieldByName('ES_HIPOTECA').AsInteger;
            CDGarReal.Post;
            Next;
           end;
          Close;
...
procedure TfrmMantenimientoGarReal.EdNumeroColocacionKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self);
end;

procedure TfrmMantenimientoGarReal.FormShow(Sender: TObject);
begin
...
procedure TfrmMantenimientoGarReal.BtnAceptarClick(Sender: TObject);
begin
...
//        EdNumeroColocacion.Text := '';
        ...
        with IBSQL2 do begin
          ...
          SQL.Add(':FECHA_FINAL_POLIZA,:CODIGO_ASEGURADORA,:ES_HIPOTECA)');
          CDGarReal.Last;
          CDGarReal.First;
          while not CDGarReal.Eof do begin
            ...
            ParamByName('ES_HIPOTECA').AsBoolean := CDGarRealES_HIPOTECA.Value;
            ExecQuery;
            CDGarReal.Next;
          end;
           Transaction.Commit;
...
procedure TfrmMantenimientoGarReal.BtnCerrarClick(Sender: TObject);
begin
        Close;
end;

procedure TfrmMantenimientoGarReal.DBGridRealEditButtonClick(Sender: TObject);
...
procedure TfrmMantenimientoGarReal.DBGridRealDrawColumnCell(Sender: TObject;
...
procedure TfrmMantenimientoGarReal.DBGridRealColExit(Sender: TObject);
begin
if DBGrid1.SelectedField.FieldName = DBCheckBox1.DataField then
    DBCheckBox1.Visible := False;
end;

procedure TfrmMantenimientoGarReal.FormCreate(Sender: TObject);
begin
        DBCheckBox1.DataSource := DSReal;
        DBCheckBox1.DataField  := 'ES_HIPOTECA';
        DBCheckBox1.Visible    := False;
...
procedure TfrmMantenimientoGarReal.DBGridRealDblClick(Sender: TObject);
begin
          CDGarReal.Edit;
          CDGarReal.FieldValues['ES_HIPOTECA'] := not(CDGarRealES_HIPOTECA.Value);
          cdgarreal.Post;
end;

procedure TfrmMantenimientoGarReal.DBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
const IsChecked : array[Boolean] of Integer =
      (DFCS_BUTTONCHECK, DFCS_BUTTONCHECK or DFCS_CHECKED);
...
procedure TfrmMantenimientoGarReal.DBGrid1ColExit(Sender: TObject);
begin
 if DBGrid1.SelectedField.FieldName = DBCheckBox1.DataField then
    DBCheckBox1.Visible := False
end;

procedure TfrmMantenimientoGarReal.DBGrid1DblClick(Sender: TObject);
begin
          CDGarReal.Edit;
          CDGarReal.FieldValues['ES_HIPOTECA'] := not(CDGarRealES_HIPOTECA.Value);
          cdgarreal.Post;
...
procedure TfrmMantenimientoGarReal.DBGrid1EditButtonClick(Sender: TObject);
var frmCalendario:TfrmCalendario;
    Columna,Fila :Integer;
    Celda:TRect;
begin
