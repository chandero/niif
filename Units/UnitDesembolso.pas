...
uses                                                  
  ...
  JvGroupBox, Menus, Math, FR_DSet, FR_DBSet, FR_Class, pr_Common, UnitDmGeneral, UnitdmColocacion, UnitDmSolicitud,
  frRtfExp, pr_TxClasses, DateUtils, JclDateTime, Provider;
...
type
  TFrmDesembolso = class(TForm)
    ...
    CDliquidacionCUOTA_NUMERO: TIntegerField;
    CDliquidacionFECHA_A_PAGAR: TDateField;
    CDliquidacionCAPITAL_A_PAGAR: TCurrencyField;
    CDliquidacionINTERES_A_PAGAR: TCurrencyField;
    ...
    btnOtrosDescuentos: TBitBtn;
    DBOtrosDescuentos: TDBGrid;
    DSOtrosDescuentos: TDataSource;
    CDSDescuentoDESCRIPCION_DESCUENTO: TStringField;
    GroupBox3: TGroupBox;
    cbFormaDesembolso: TComboBox;
    panelBancoDesembolso: TPanel;
    lstBancos: TDBLookupComboBox;
    Label13: TLabel;
    Label14: TLabel;
    edCheque: TEdit;
    IBQBancos: TIBQuery;
    ...
    IBQCodigo: TIBQuery;
    EdValorAporte: TJvCurrencyEdit;
    EdValorAsobancaria: TJvCurrencyEdit;
    Label15: TLabel;
    ...
    procedure btnOtrosDescuentosClick(Sender: TObject);
    procedure cbFormaDesembolsoChange(Sender: TObject);
    procedure EdValorAporteKeyPress(Sender: TObject; var Key: Char);
    procedure EdValorAsobancariaKeyPress(Sender: TObject; var Key: Char);
    procedure EdFechaDesembolsoExit(Sender: TObject);
    procedure EdFechaDesembolsoChange(Sender: TObject);
  private
    vvalorpuntos: single;
...
begin
        ...
        IBtipoid.Database := dmGeneral.IBDatabase1;
        IbTipocuenta.Database := dmGeneral.IBDatabase1;
        IBcdat.Database := dmGeneral.IBDatabase1;
        IBCdat.Transaction := dmGeneral.IBTransaction1;
        ...
        IBSQL12.Database := dmGeneral.IBDatabase1;
        IBgrupo1.Database := dmGeneral.IBDatabase1;
        IBgrupo2.Database := dmGeneral.IBDatabase1;
        IBQuery1.Database := dmGeneral.IBDatabase1;
        IBQuery2.Database := dmGeneral.IBDatabase1;
        IBQuerycontable.Database := dmGeneral.IBDatabase1;
        IBDescuentos.Database := dmGeneral.IBDatabase1;
        IBDescuentos.Transaction := dmGeneral.IBTransaction1;
        IBentidad.Database := dmGeneral.IBDatabase1;
        IBentidad.Transaction := dmGeneral.IBTransaction1;
        ...
        IBhorario.Transaction := dmGeneral.IBTransaction1;
        ...
        IBConsecB.Database := dmGeneral.IBDatabase1;
        IBConsecutivo.Database := dmGeneral.IBDatabase1;
        IBExtracto.Database := dmGeneral.IBDatabase1;
        IBExtracto.Transaction := dmGeneral.IBTransaction1;
        IBExtractoInt.Database := dmGeneral.IBDatabase1;
        IBExtractoInt.Transaction := dmGeneral.IBTransaction1;
        ...
        IBSQL13.Database := dmGeneral.IBDatabase1;
        ...
        IBTransaction9.DefaultDatabase := dmGeneral.IBDatabase1;
        IBTConsecutivo.DefaultDatabase := dmGeneral.IBDatabase1;
        IBTSolicitud.DefaultDatabase := dmGeneral.IBDatabase1;
        IBtranreporte.DefaultDatabase := dmGeneral.IBDatabase1;
        IBTgrupos.DefaultDatabase := dmGeneral.IBDatabase1;
...
begin
        ...
        begin
        {
          ...
          // Cargar descuentos a aplicar en cuotas
             IBQDescuento.ParamByName('ID_AGENCIA').AsInteger := Agencia;
             IBQDescuento.ParamByName('ID_SOLICITUD').AsString := vIdSolicitud;
             IBQDescuento.Open;
             if not CDSDescuento.Active then  CDSDescuento.Active := True;
...
//            EdCuentaAportes.Caption := IntToStr(vCtaAportes);
            ...
            begin
              ...
              begin
                if FieldByName('ID_ESTADO').AsInteger = 1 then
                  ...
                  begin
                     MessageDlg('El Vencimiento del CDAT es Menor que el Vencimiento del Crédito' + #13 + ' Plazo Máximo Pemitido por el CDAT ' + IntToStr(vDias) + ' Dias a Fecha Hoy',mtInformation,[mbok],0);
                     cmdnuevo.Click;
                     Exit;
                  end;
            ...
            //vTasaMora := vTasaEfectiva;
            vTasaMora := vTasaMaxClasificacion;
...
begin
        ...
        //vTasaMora := vTasaEfectiva;// valor por omision
...
//vivienda******************************
{
          ...
          vAmortizaCapital := vAmortizaInteres;
          EdAmortizaCap.Text := FormatCurr('#0',vAmortizaCapital);
          vValorCuota := CuotaFija(vDesembolso,Plazo,vTasaEfectiva,vAmortizaInteres, vRedondeo);
        end
        else
        begin
          ...
          vValorCuota := (vDesembolso * vTasaNominal/100) / (Plazo/vAmortizaInteres)
...
begin
        ...
        DBOtrosDescuentos.Visible := False;
...
//VidColocacion := '20050003598';
        ...
        begin
             ...
             CalcularDescuentoPorCuota(CDliquidacion,CDSDescuento, CDSADescontar, vDesembolso);
...
begin
        ...
        begin
           ...
           //Quitar proceso cuando se actualice el proceso de causación
{           if CDgarantiareal.RecordCount > 0 then
           begin
  ...
  try
        ...
        begin
           with dmColocacion.IBQuery do begin
            ...
            ParamByName('ID_AGENCIA').AsInteger := Agencia;
            ParamByName('ID_COLOCACION').AsString := vIdColocacion;
            ParamByName('CODIGO').AsString := recupera_codigo(59);
            ParamByName('DEBITO').AsCurrency := 0;
            ParamByName('CREDITO').AsCurrency := vAportes;
        ...
        begin
           with dmColocacion.IBQuery do begin
            ...
            ParamByName('ID_AGENCIA').AsInteger := Agencia;
            ParamByName('ID_COLOCACION').AsString := vIdColocacion;
            ParamByName('CODIGO').AsString := recupera_codigo(60);
            ParamByName('DEBITO').AsCurrency := 0;
            ParamByName('CREDITO').AsCurrency := vAsobancaria;
            ParamByName('FECHA_Y_HORA').AsDateTime := vfechahora;
            ExecSQL;
            Close;
            vCaja := vCaja - vAsobancaria;
          end;
        end;
        ...
        begin
            ...
            begin
              if DiasEntre(EdFechaPago.Date,IBQuery.FieldbyName('FECHA_A_PAGAR').AsDateTime) <= 360 then
                 vCortoPlazo := vCortoPlazo + IBQuery.FieldByName('CAPITAL_A_PAGAR').AsCurrency
              else
                 vLargoPlazo := vLargoPlazo + IBQuery.FieldByName('CAPITAL_A_PAGAR').AsCurrency;
            Next;
            end;
            Close;
        end;

        if (vCortoPlazo + vLargoPlazo) <> vDesembolso then
        begin
           vCortoPlazo := vDesembolso;
           vLargoPlazo := 0;
        end;
          ...
          begin
            ...
            if vDescuento > 0 then begin
              ...
              // extracto interes por fecha de pago
                  ...
                  begin
                    case cbFormaDesembolso.ItemIndex of
                       0: begin
                            IBQCodigo.SQL.Clear;
                            IBQCodigo.SQL.Add('select CODIGO from "col$codigospucbasicos" ');
                            IBQCodigo.SQL.Add('where ID_CODIGOPUCBASICO = 1');
                            IBQCodigo.Open;
                            vCodigoPuc := IBQCodigo.FieldByName('CODIGO').AsString;
                          end;
                       1: begin
                            vCodigoPuc := IBQBancos.FieldByName('CODIGO').AsString;
                          end;
                       2: begin
                            vCodigoPuc := IBQBancos.FieldByName('CODIGO').AsString;
                            vCheque := edCheque.Text;
                          end;
                    end;
...
{Agregar al extracto}
                ...
                begin
                  ...
                  begin
                    // vCodigoPuc depende de la seleccion al desembolsar
                    case cbFormaDesembolso.ItemIndex of
                       0: begin
                            IBQCodigo.SQL.Clear;
                            IBQCodigo.SQL.Add('select CODIGO from "col$codigospucbasicos" ');
...
begin
{
        if edfechapago.Date < EdFechaDesembolso.Date then
        begin
            EdFechaPago.Date := EdFechaDesembolso.Date;
        end;
        }
        VfechaPago := EdFechaPago.Date;
        vFechaVencimiento := EdFechaPago.Date;
        vFechaVencimiento := CalculoFecha(vFechaVencimiento,vPlazo+vPeriodoGracia);
        DateTime_SetFormat(EdFechaVencimiento.Handle,'yyyy/MM/dd');
        EdFechaVencimiento.Date := vFechaVencimiento;
...
begin
        ...
        begin
          ...
          Close;
          SQL.Clear;
          SQL.Add('insert into "col$desembolso" (ID_AGENCIA,ID_COLOCACION,HORA_DESEMBOLSO, ID_SOLICITUD)');
          SQL.Add('values (:ID_AGENCIA,:ID_COLOCACION,:HORA_DESEMBOLSO, :ID_SOLICITUD)');
          ParamByName('ID_AGENCIA').AsInteger := Agencia;
          ParamByName('ID_COLOCACION').AsString := VidColocacion;
...
begin
        ...
        GrupoInformacion.Caption := 'Información Bancoldex';
end;
...
begin
        ...
        GrupoInformacion.Caption := 'Información Cuentas Adicionales';

end;

procedure TFrmDesembolso.GrabarDescuentos;
begin
     with dmColocacion.IBQuery do
     begin
        Close;
        SQL.Clear;
        SQL.Add('insert into "col$colocaciondescuento" (ID_COLOCACION, ID_DESCUENTO, ID_AGENCIA) values (:ID_COLOCACION, :ID_DESCUENTO, :ID_AGENCIA)');
        CDSDescuento.First;
        while not CDSDescuento.Eof do
        begin
               ParamByName('ID_COLOCACION').AsString := VidColocacion;
               ParamByName('ID_DESCUENTO').AsInteger := CDSDescuento.FieldByName('ID_DESCUENTO').AsInteger;
               ParamByName('ID_AGENCIA').AsInteger := Agencia;
               ExecSQL;
               CDSDescuento.Next;
        end
        ...
        begin
           ...
           begin
             ...
             begin
                ...
                // AGREGAR LOS DUEÑOS DE LA GARANTIA
                Close;
                SQL.Clear;
                ...
                // FIN AGREGAR DUEÑOS DE LA GARANTIA
...
procedure TFrmDesembolso.btnOtrosDescuentosClick(Sender: TObject);
begin
        LimpiaInformacion;
        DBOtrosDescuentos.Visible := True;
        GrupoInformacion.Caption := 'Otros Descuentos en Cuota';

end;

procedure TFrmDesembolso.cbFormaDesembolsoChange(Sender: TObject);
begin
        case cbFormaDesembolso.ItemIndex of
                0:  begin
                        panelBancoDesembolso.Visible := False;

                    end;
                1:  begin
                        panelBancoDesembolso.Visible := True;
...
procedure TFrmDesembolso.EdValorAporteKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolso.EdValorAsobancariaKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolso.EdFechaDesembolsoExit(Sender: TObject);
begin
        vFechaDesembolso := EdFechaDesembolso.Date;
        EdFechaPago.Date := vFechaDesembolso;
        EdFechaPago.MinDate := vFechaDesembolso;
        vFechaPago := vFechaDesembolso;
end;

procedure TFrmDesembolso.EdFechaDesembolsoChange(Sender: TObject);
begin
        vFechaDesembolso := EdFechaDesembolso.Date;
        EdFechaPago.Date := vFechaDesembolso;
        vFechaPago := vFechaDesembolso;
