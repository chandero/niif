...
uses
  ...
  IBCustomDataSet, IBQuery, pr_Common, pr_TxClasses, IBSQL, DateUtils, Math,  UnitDmGeneral, UnitDmColocacion,
  DBClient, JvEdit, JvTypedEdit, Grids, JvStringGrid, JvMaskEdit,
...
type
  TfrmLiquidacionCarteraJudicial = class(TForm)
    ...
    IBEfectivo: TIBSQL;
    edFecha: TJvDatePickerEdit;
    Label11: TLabel;
    EdComisionBanco: TJvCurrencyEdit;
    Label12: TLabel;
    EdSeguroVida: TJvCurrencyEdit;
    ...
    procedure EdComisionBancoExit(Sender: TObject);
    procedure EdSeguroVidaExit(Sender: TObject);
  private
...
var
  ...
  vComisionBanco          : Currency;
  vSeguroVida             : Currency;
  vTipoInteres            : string;
  TipoInteres             : Integer;
  vPuntosAdic             : Double;
  TasaC                   : Double;
...
uses UnitGlobales, UnitBuscarColocacion,
...
begin
        ...
        IBExtractoDet.Database := dmGeneral.IBDatabase1;
        ...
        IBEfectivo.Database := dmGeneral.IBDatabase1;
        ...
        IBExtractoDet.Transaction := dmGeneral.IBTransaction1;
        ...
        IBEfectivo.Transaction := dmGeneral.IBTransaction1;
...
begin
        ...
        begin
           ...
           ExecQuery;
           CodigoHonorarios := FieldByName('CODIGO').AsString;
...
// Coloco Comision Banco / Seguro

        IBQDescuento.Filtered := False;

        if vComisionBanco > 0 then
        begin
           New(AR);
           AR^.CuotaNumero := 0;
           IBQDescuento.Locate('ID_DESCUENTO',VarArrayOf([1]),[loCaseInsensitive]);
           AR^.CodigoPuc :=  IBQDescuento.FieldByName('CODIGO').AsString;
           AR^.FechaInicial := fFechaActual;
           AR^.FechaFinal := fFechaActual;
           AR^.Dias := 0;
           AR^.Tasa := 0;
           AR^.Debito := 0;
           AR^.Credito := vComisionBanco;
           AR^.EsCapital := False;
           AR^.EsCausado := False;
           AR^.EsCorriente := False;
           AR^.EsVencido := False;
        ...
        if vSeguroVida > 0 then
        begin
           New(AR);
           AR^.CuotaNumero := 0;
           IBQDescuento.Locate('ID_DESCUENTO',VarArrayOf([2]),[loCaseInsensitive]);
           AR^.CodigoPuc :=  IBQDescuento.FieldByName('CODIGO').AsString;
           AR^.FechaInicial := fFechaActual;
           AR^.FechaFinal := fFechaActual;
           AR^.Dias := 0;
...
// Fin Comision Banco / Seguro

        if vhonorarios <> 0 then
         begin
           New(AR);
           AR^.CuotaNumero := 0;
           AR^.CodigoPuc := CodigoHonorarios;
           AR^.FechaInicial := fFechaActual;
           AR^.FechaFinal := fFechaActual;
           AR^.Dias := 0;
           AR^.Tasa := 0;
...
begin
      with IBQuery do
        begin
          ...
          ParamByName('ID_COLOCACION').AsString := vColocacion;
          ParamByName('FECHA_EXTRACTO').AsDate := edFecha.Date;
      ...
      begin
       ...
       if (EsCapital.Checked = True) and (EsIntereses.Checked = True) then
        begin
          ...
          IBConsulta.ParamByName('FECHA_EXTRACTO').AsDate := edFecha.Date;
          IBConsulta.ExecQuery;
          IBConsulta.Close;
...
var Tabla:string;
    ...
    Cuenta : string;
begin
        If IBCodigo.Transaction.InTransaction then
          IBCodigo.Transaction.Commit;
        IBCodigo.Transaction.StartTransaction;
        ...
        for I := 0 to (MyCuotasLiq.Lista.Count - 1) do
         begin
           ...
           Report.Variables.ByName['Hoy'].AsDateTime := EdFecha.Date;
           Report.Variables.ByName['FechaCorte'].AsDateTime := EdFecha.Date;
...
begin
  ...
  if not IBQDescuento.Active then IBQDescuento.Open;
     ...
     try
      ...
      if  MyCuotasLiq.Lista.Count > 0 then
       for I := 0 to MyCuotasLiq.Lista.Count -1 do
        ...
        if vhonorarios > 0 then begin
           ...
           ParamByName('FECHA_MOVIMIENTO').AsDateTime := EdFecha.Date;
      ...
      with IBQuery1 do
       begin
         ...
         if vNacional > 0 then
           ParamByName('DESCRIPCION').AsString := 'Descuento de Cartera, Pagaré No.' + vColocacion + ' ' + 'Consig. Nal' + ' ' + DateToStr(FechaConsignacion)
         else
           ParamByName('DESCRIPCION').AsString := 'Descuento de Cartera, Pagaré No.' + vColocacion;
           ...
           while not Eof do begin
            if i > 1 then
              GridCuentas.RowCount := GridCuentas.RowCount + 1;
                ...
                IBSQL2.ParamByName('ANO').AsString := LeftStr(DateToStr(EdFecha.Date),4);
...
begin
          ...
          vTotalCuentas := vTotalAportes + vTotalAhorros + vTotalJuvenil + vNacional + vEfectivo;
end;
...
begin
      ...
      vTotalCuentas := vTotalAportes + vTotalAhorros + vTotalJuvenil + vNacional + vEfectivo;
      if EdNacional.Value > 0 then DBLCBBancos.SetFocus;
...
procedure TfrmLiquidacionCarteraJudicial.EdEfectivoExit(Sender: TObject);
begin
        vEfectivo := EdEfectivo.Value;
        vTotalCuentas := vTotalAportes + vTotalAhorros + vTotalJuvenil + vNacional + vEfectivo;
end;

procedure TfrmLiquidacionCarteraJudicial.EdEfectivoEnter(Sender: TObject);
begin
        EdEfectivo.SelectAll;
end;

procedure TfrmLiquidacionCarteraJudicial.EdComisionBancoExit(
  Sender: TObject);
begin
        vComisionBanco := EdComisionBanco.Value;
        TGeneral := vAbonoCapital + vAbonoInteresCausado + vAbonoInteresContingente + vAbonoInteresCorriente +
                    vAbonoInteresAnticipado + vAbonoInteresMora + vInteresCobradoCausado + vInteresCobradoCorriente - vInteresDevuelto + vPapeleria + vExAsociados + + vhonorarios + vcostas + vComisionNal +
                    vComisionBanco + vSeguroVida;
...
procedure TfrmLiquidacionCarteraJudicial.EdSeguroVidaExit(Sender: TObject);
begin
        vSeguroVida := EdSeguroVida.Value;
        TGeneral := vAbonoCapital + vAbonoInteresCausado + vAbonoInteresContingente + vAbonoInteresCorriente +
                    vAbonoInteresAnticipado + vAbonoInteresMora + vInteresCobradoCausado + vInteresCobradoCorriente - vInteresDevuelto + vPapeleria + vExAsociados + + vhonorarios + vcostas + vComisionNal +
                    vComisionBanco + vSeguroVida;
